...
class AutocompleteEntry:
    ...
    def _on_arrow_down(self, event):
        if self.listbox and self.listbox.size() > 0:
            ...
            next_idx = min(
                current[0] + 1, self.listbox.size() - 1) if current else 0
...
class FolderProcessorApp:
    def __init__(self, root):
        ...
        if HAS_PILLOW:
            ...
            print(
                f"[ICON SETUP] Loaded {len([i for i in self.icons.values() if i])} icons")

            # Debug: List icons folder contents
            icons_dir = Path(__file__).parent / "icons"
            if icons_dir.exists():
                print(
                    f"[FILES] Contents of icons/: {[f.name for f in icons_dir.iterdir()]}")
            else:
                print(
                    f"[FILES] icons/ folder does not exist (fallback icons will be used)")
        else:
            print("[ICON SETUP] Pillow not available, no icons")

        # --- UI Layout ---
        title_label = tk.Label(
            root, text="Switch Save Converter", font=("Arial", 16, "bold"))
        title_label.pack(pady=5)

        mode_frame = tk.Frame(root)
        mode_frame.pack(pady=5)
        tk.Label(mode_frame, text="Mode:", font=(
        ...
        tk.Radiobutton(
            ...
            variable=self.mode,
            value="manual",
            command=self.update_ui_state).pack(
            side=tk.LEFT,
            padx=5)
        tk.Radiobutton(
            mode_frame,
            text="Newest Save File",
            variable=self.mode,
            value="newest",
            command=self.update_ui_state).pack(
            side=tk.LEFT,
        ...
        tk.Label(left_frame, text="Source Format:", font=(
            "Arial", 10)).pack(anchor=tk.W, pady=5)
        self.source_var = tk.StringVar(value=self.manual_formats[0])
        self.source_combo = ttk.Combobox(
            left_frame,
            textvariable=self.source_var,
            values=self.manual_formats,
            state="",
            width=15)
        self.source_combo.pack(pady=5)
        self.source_combo.bind("<<ComboboxSelected>>",
                               lambda e: self.update_dropdowns())
        ...
        tk.Label(right_frame, text="Target Format:", font=(
            "Arial", 10)).pack(anchor=tk.W, pady=5)
        self.target_var = tk.StringVar(value=self.manual_formats[1])
        self.target_combo = ttk.Combobox(
            right_frame,
            textvariable=self.target_var,
            values=self.all_formats,
            state="",
            width=15)
        self.target_combo.pack(pady=5)
        self.target_combo.bind("<<ComboboxSelected>>",
                               lambda e: self.update_dropdowns())
        ...
        self.arrow_label = tk.Label(arrow_frame, text="➜", font=(
            "Arial", 24, "bold"), fg="#4CAF50", bg="white", width=3)
        self.arrow_label.pack(expand=True)

        select_frame = tk.Frame(root)
        select_frame.pack(pady=10)
        self.path_entry = tk.Entry(
            select_frame, width=55, relief=tk.SUNKEN, fg="gray")
        self.path_entry.insert(0, "No folder selected")
        self.path_entry.pack(side=tk.LEFT, padx=5)
        self.path_entry.bind("<Return>", self._on_path_enter)
        self.select_btn = tk.Button(
            select_frame, text="Browse...", command=self.browse_folder)
        self.select_btn.pack(side=tk.LEFT, padx=5)

        game_frame = tk.Frame(root)
        game_frame.pack(pady=5)
        tk.Label(game_frame, text="Game:", font=(
            "Arial", 10)).pack(side=tk.LEFT, padx=5)
        ...
        self.game_hint = tk.Label(
            game_frame,
            text="(Type to search game title)",
            fg="gray",
            font=(
                "Arial",
                9))
        ...
        tk.Label(title_id_frame, text="Title ID:", font=(
            "Arial", 10)).pack(side=tk.LEFT, padx=5)
        self.title_id_entry = tk.Entry(
            title_id_frame,
            textvariable=self.title_id_var,
            width=25,
            relief=tk.SUNKEN,
            fg="gray")
        self.title_id_entry.insert(0, "Auto-detected on conversion")
        self.title_id_entry.pack(side=tk.LEFT, padx=5)
        self.title_id_clear_btn = tk.Button(
            title_id_frame,
            text="Clear",
            command=self._clear_title_id_override)
        ...
        self.validation_warning = tk.Label(
            validation_frame,
            text="Check this if you wish to manually name required saves yourself,"
                + " or if you have issues with validation",
            font=(
                "Arial",
                9))
    ...
    def _on_path_enter(self, event):
        ...
        if not text or text == "No folder selected":
            messagebox.showwarning(
                "Empty Path", "Please enter a valid file or folder path.")
            return
        path_obj = Path(text)
        if not path_obj.exists():
            messagebox.showerror(
                "Path Not Found", f"The path does not exist:\n{text}")
            return
        ...
        if mode == "manual":
            if source_format in ["Eden", "JKSV"] and path_obj.is_file():
                if path_obj.suffix.lower() != ".zip":
                    messagebox.showwarning(
                        "Invalid File", "Eden/JKSV sources must be .zip files.")
                    return
            ...
            if not path_obj.is_dir():
                messagebox.showwarning(
                    "Invalid Path",
                    "In Newest Save File mode, the path must be a directory.")
                return
            self.selected_path = path_obj
            self._set_path_display(str(path_obj), "black")
            self.process_btn.config(state=tk.NORMAL)
    ...
    def open_mapping_manager(self):
        ...
        def _refresh_mappings_in_window():
            ...
            refresh_treeview()
            status.config(
                text=f"Loaded {len(mapper.id_to_name)} mappings", fg="green")
            ...
            if tid and name:
                if mapper.add_manual_override(tid, name):
                    treeview.insert("", "end", values=(
                        tid, name), tags=(mapper._normalize_tid(tid),))
        ...
        def remove_selected():
            ...
            if not selection:
                status.config(
                    text="Please select an override to remove", fg="red")
                return
            ...
            if deleted_count > 0:
                status.config(
                    text=f"Removed {deleted_count} override(s)", fg="green")
        ...
        for tid, name in mapper.get_manual_overrides():
            treeview.insert("", "end", values=(tid, name),
                            tags=(mapper._normalize_tid(tid),))
        ...
        tk.Button(
            refresh_btn_frame,
            text="Refresh Remote Data",
            command=_refresh_mappings_in_window,
            bg="#2196F3").pack(
            side=tk.LEFT,
            padx=5)
        ...
        tk.Button(close_btn_frame, text="Close",
                  command=refresh_window.destroy).pack()
    ...
    def update_ui_state(self):
        ...
        if mode == "manual":
            ...
            self._set_path_display(
                "Select the specific save folder or zip file.")
            ...
            self._set_path_display(
                "Select the parent directory. The newest save inside will be converted.")
            if self.source_var.get() not in ["Checkpoint", "Eden", "JKSV"]:
                self.source_var.set("Checkpoint")
    ...
    def update_dropdowns(self):
        ...
        available_targets = [
            opt for opt in self.all_formats if opt != source_val]
        ...
        if mode == "manual":
            if source_format == "Checkpoint":
                folder_selected = filedialog.askdirectory(
                    title="Select Checkpoint Save Folder")
                if folder_selected:
                    self.selected_path = Path(folder_selected)
                    self._set_path_display(str(self.selected_path), "black")
                    self.process_btn.config(state=tk.NORMAL)
                    self.status_var.set(
                        f"Selected Folder: {self.selected_path.name}")
            elif source_format in ["Eden", "JKSV"]:
                file_selected = filedialog.askopenfilename(
                    title=f"Select {source_format} Save Zip", filetypes=[
                        ("Zip files", "*.zip"), ("All files", "*.*")])
                if file_selected:
                    path_obj = Path(file_selected)
                    if path_obj.suffix.lower() != ".zip":
                        messagebox.showwarning(
                            "Invalid File", "Please select a .zip file.")
                        return
                    self.selected_path = path_obj
                    self._set_path_display(str(path_obj), "black")
                    self.process_btn.config(state=tk.NORMAL)
                ...
                messagebox.showerror(
                    "Error", "Invalid Source Format selected.")
                return
        else:
            folder_selected = filedialog.askdirectory(
    ...
    def process_folder(self):
        ...
        if source_format not in valid_formats:
            messagebox.showerror(
                "Invalid Source",
                f"'{source_format}' is not a valid format."
                + "\nPlease select from: {', '.join(valid_formats)}")
            return
        if target_format not in valid_formats:
            messagebox.showerror(
                "Invalid Target",
                f"'{target_format}' is not a valid format."
                + "\nPlease select from: {', '.join(valid_formats)}")
            return
        ...
        try:
            ...
            zip_filename, original_name, status_msg = converter.convert(
                source_format, target_format, title_id=title_id,
                    is_auto_mode=is_auto, remove_tempfiles_when_done=remove_tempfiles)
            self.status_var.set(f"{status_msg} -> {target_format}")
            self.root.update()
            cleanup_note = " (temp files removed)" if remove_tempfiles else ""
            self.status_var.set(
                f"Success! Created {zip_filename}{cleanup_note}")
            messagebox.showinfo(
                "Success",
...
if __name__ == "__main__":
    try:
        ...
        messagebox.showerror(
            "Tkinter Error",
            f"Could not initialize GUI. Ensure 'python-tk' is installed.\nError: {e}")
