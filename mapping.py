...
class TitleIdMapper:
    ...
    def _load_cache(self) -> bool:
        ...
        try:
            with open(self.CACHE_FILE, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, dict):
                    ...
                    for entry in data:
                        if isinstance(entry, dict):
                            tid = entry.get('id') or entry.get(
                                'title_id') or entry.get('program_id')
                            name = entry.get('name') or entry.get(
                                'title') or entry.get('game_name')
                            if tid and name:
                                norm_tid = self._normalize_tid(tid)
                                if norm_tid:
                                    self._add_mapping(norm_tid, name)
                    ...
                    print(
                        f"[TitleIdMapper] Unexpected cache format: {type(data)}")
                    return False
                ...
                print(
                    f"[TitleIdMapper] Loaded {len(self.id_to_name)} mappings from cache")
                return True
    ...
    def fetch_remote_data(self) -> bool:
        ...
        try:
            ...
            print(
                f"[TitleIdMapper] Updated from remote source: {len(self.id_to_name)} mappings")
            return True
            ...
            try:
                ...
                for tid, name in raw_id_to_name.items():
                    ...
                    if norm_tid and name:
                        ...
                        self.manual_reverse_overrides[name.strip(
                        ).lower()] = norm_tid

                print(
                    f"[TitleIdMapper] Loaded {len(self.manual_overrides)} manual overrides")
    ...
    def save_manual_overrides(self) -> None:
        ...
        try:
            ...
            print(
                f"[TitleIdMapper] Saved {len(self.manual_overrides)} manual overrides")
